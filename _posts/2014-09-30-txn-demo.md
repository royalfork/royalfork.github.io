...
Bitcoin transactions are a bit like bank checks.  Bank checks display a bank account number, which reveals where the about-to-be-transferred funds come from, they have a "payee" field, which identifies the recipient of the transferred funds, and they have a signature, which serves to proove that the owner of the account authorized the transfer of funds. 

Bitcoin transactions are composed of 2 lists:

* a list of "outputs", each output says:
  - how much bitcoin is transferred
  - who recevies the bitcoin transfer
...
Change must be given in these transactions if the sum of the inputs is greater than the amount being sent to the recipient.  After adding the "change" output, i the sum of the inputs is greater than the sum of the outputs, the remainder is collected by the miner who confirms your transaction, which is called the "fee". Naturally, the sum of all bitcoins in the inputs must be greater than or equal to the sum of all bitcoins in the outputs, or else your transaction will get rejected.

# Outputs

Beware: the following examples show hex digits.  Because all data on computers is a series of 1's and 0's, hex lets us conveniently talk about those 1's and 0's without actually printing all the 1's and 0's, is hard to read.  
...
<p>
I've generated a bitcoin address below.  If you're curious/confused, click <a href="/2014/08/11/graphical-address-generator#testnet">here</a> to see how these were made.
</p>
...
<div class="post-content" ng-show="utxo">
  ...
  <p>
    It's identified by it's double hash:
  </p>
  ...
  <p>
    You can check it out on the block chain: <a target="_blank" href="http://btc.blockr.io/tx/info/b138360800cdc72248c3ca8dfd06de85913d1aac7f41b4fa54eb1f5a4a379081">here</a>.
  </p>
  ...
  <table>
    <tbody>
      ...
      <tr>
        ...
        <td>This is the <b>locking script</b>, which describes the specific information required to spend the bitcoins in this ourput.  As you can see, "// key.addr_dec //" matches our decoded address.</td>
      </tr>
    </tbody>
  </table>
...
<p>
We've established that outputs are used to transfer funds from an old owner to a new owner, and that the new owner uses these outputs as inputs into new transactions.  Given that the details of all outputs are public, how do we ensure that only the intended recipient can use the output, and nobody else?
</p>

<p>
When outputs are created, they come with a "locking script".  This locking script is essentially a mathematical equation with the promise that anyone who can successfully solve the mathematical equation is authorized to spend the funds associated with this output.  An input provides the answer to that mathematical equation in an "unlocking script" when it references the output.  Before processing transactions, miners check to ensure that an input's unlocking script successfully solves the referenced output's locking script.  Technically, to verify if an "unlocking script" correctly unlocks a "locking" script, we join both scripts together, and determine whether the resulting expression is true or false.
</p>
...
<table>
  <tbody>
    ...
    <tr>
      ...
      <td>There can be several outputs in the referenced transaction, this identifies which of those outputs we're specifically using.</td>
    </tr>
    <tr>
...
<p>Congratulation, we have just created our very own transaction!  Because this is real bitcoin, I've already spent the output referenced by in the input in this transaction, so the above transaction will be rejected as an attempted double spend.  Questions or comments always welcome.</p> 
